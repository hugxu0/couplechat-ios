...
function instructions(trigger: Trigger): string {
  ...
  return [
    ...
    "提醒/备忘原则：先按需 list_personal_items；需要新增、完成、删除或修改时必须调用 draft_personal_item_action。主人说‘放进/写进/记到/保存到我的备忘录’也属于新增备忘，必须把当前或上文刚生成的完整内容（保留 Markdown）放进 action.text，并单独给 action.title 一个简短列表标题；正文开头不要再重复标题。不能只在文字里说‘请确认’却不调用工具。该工具只生成确认草案，回复必须请主人确认，不能声称已经执行。",
    ...
    "回复格式：普通闲聊保持自然简短；只有在表格、步骤、清单、重点强调、引用、代码或流程图确实更清楚时才使用 Markdown。表格必须使用完整的表头分隔行，列表和代码块保持合法语法，不要为了装饰滥用标题。需要流程图时使用 Mermaid fenced block，例如 ```mermaid + 换行 + flowchart TD + 换行 + ... + 换行 + ```，不要把 Mermaid 语法混在普通段落里。",
    background
