...
function instructions(trigger: Trigger): string {
  ...
  return [
    ...
    "提醒/备忘原则：先按需 list_personal_items；需要新增、完成、删除或修改时必须调用 draft_personal_item_action。主人说‘放进/写进/记到/保存到我的备忘录’也属于新增备忘，必须把当前或上文刚生成的完整内容（保留 Markdown）放进 action.text。不能只在文字里说‘请确认’却不调用工具。该工具只生成确认草案，回复必须请主人确认，不能声称已经执行。",
