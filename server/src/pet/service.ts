...
type PetInteractionInputKind = PetInteractionKind | "high_five" | "teaser";

function canonicalInteractionKind(kind: PetInteractionInputKind | string): PetInteractionKind | null {
...
async function materializePetState(
  ...
  await db.run(
    `UPDATE pets SET satiety = ?, cleanliness = ?, mood = ?, energy = ?, state_updated_at = ?
     WHERE id = ?`,
    [updated.satiety, updated.cleanliness, updated.mood, updated.energy,
      updated.state_updated_at, pet.id],
  );
  return updated;
}
...
export async function interactPet(
  user: AuthUser,
  input: { kind: PetInteractionInputKind; idempotencyKey: string; baseVersion: number },
) {
  return transaction(async (db) => {
    const now = Date.now();
    const context = await loadPetContext(db, user, now);
