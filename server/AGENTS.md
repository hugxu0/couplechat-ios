...
- PostgreSQL 是事实源。migration 只追加，不修改 v1-v31；新 migration 必须可重复检查，并在隔离恢复库核验。
...
- 任何同步事件都必须通过统一写入边界，并在分配 `server_seq` 前取得同一 transaction-level advisory lock；修改该边界前先阅读 `Docs/ARCHITECTURE.md` 并在隔离 PostgreSQL 环境完成并发提交核验。
