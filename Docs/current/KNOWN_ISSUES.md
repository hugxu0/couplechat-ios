...
> 审查基线：2026-07-16，源码 `f1e69c446b614301859cb9d06d9b504af061172d`。这里只保留尚未解决或需要现场核验的问题；完成后应在同一提交更新状态。
...
### OPS-005：备份验证曾漏掉现行 Memory 表（代码已修复，待演练）
...
- **剩余证据**：尚未在目标 Linux 环境和真实生产副本上完成端到端恢复演练。需用至少一个现行 schema v31 备份证明完整恢复、全表计数、两条序列和媒体抽样均通过，之后才能关闭本项。

### OPS-006：旧备份曾在新备份成功前被清理（代码已修复，待演练）
...
- **剩余证据**：尚未在目标 Linux 环境做失败注入，也未做真实生产恢复演练；脚本还不能自动确认加密离机副本。发布流程必须在私有 runbook 另行确认离机副本，且不得把 `best_effort` 备份作为 migration 门禁。
...
### BUILD-001：新 IPA 证据链等待远程验收

仓库已实现同 SHA reusable 质量门禁、固定 SHA 的 Action、带 commit/run/attempt/version/hash 的 metadata，以及按完整 commit 下载。下载器会核对 workflow 数据库 ID/路径、run attempt、实际 IPA `Info.plist`、必需资源、SHA-256 和签名残留，并以安全 staging 原子发布到本机目录。由于本机是 Windows，仍需推送后完成一次 GitHub macOS runner 的成功构建、重跑 attempt 验证，并用新下载脚本回取，之后才能关闭本项。
