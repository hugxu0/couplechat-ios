...
- 主题、深浅模式、头像、设备、Bark、收藏、存储和 Memory 控制中心。
...
- 当前 schema 为 v24；v1–v24 迁移必须保留且不可改写，后续只能追加。
- 公聊事件发往 `couple:<id>`，私人事件发往 `account:<id>`。
- 数据访问集中在 `db/`，列表游标统一使用 `src/utils/cursor.ts`。
- TypeScript 启用未使用代码检查；撤回执行完整级联删除，宠物奖励和提醒投递保持事务幂等。
