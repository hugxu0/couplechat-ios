...
- SQLite 消息缓存、可靠发送队列、从云端最新页开始核对的完整历史同步、媒体缓存和 Sync cursor/ack。同步状态以本地/云端条数一致为完成条件，离开存储页不会取消任务。
...
- 纪念日、服务端权威聊天统计、多本共同相册、朋友圈式分组动态、相册直接上传和那年今日。
- 共享/私人日历、提醒、备忘、完成、删除和版本冲突处理。
- 共享提醒通知双方设备，私人提醒只通知创建者设备。
- 服务端持久化的大橘状态、互动冷却、3D 模型、日夜场景、具有自身观察与思考且不自带日期的大橘日记和 AI 私聊。
