...
- R1.1 代码和自动化测试已完成：历史同步任务移到 App 级协调器，离开存储页面不再取消；仍需安装 IPA 做真机生命周期验收，因此状态保持“进行中”。
- R1.2 尚未开始：失败消息的重新发送与删除是下一项实现任务。
...
1. 确认用户是否已安装 `CoupleChat-unsigned-205`，等待用户给出 R1.1 真机验收结果。
2. 通过则标记 R1.1 已验收，然后严格执行 R1.2；未通过则只修 R1.1，不开始 R1.2。
