...
整理器按游标读取主人文字消息。新消息经过 8 秒合并窗口后即触发整理，不再必须凑满 30 条；30 条仅是单轮上限，满批次会自动继续。整理器校验变更类型、主体、目标记忆和证据，再在事务中写入并推进游标。无效 JSON、缺失证据或写入失败都不能推进游标。
...
仅在迁移到 v25 的隔离恢复库运行调试服务后打开 `http://127.0.0.1:8080/ai-debug`。不得用 `npm run dev:cloud-db` 直接写生产库。调试页支持：
