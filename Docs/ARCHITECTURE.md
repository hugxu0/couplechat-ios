...
图片、视频封面、语音和文件预览都属于可重建缓存，不是离线事实源。图片磁盘上限 1 GiB、语音 256 MiB、文件预览 512 MiB；超过上限按最后访问时间回收到 85%。图片网络读取最多四并发，语音/文件最多两并发；全屏图片只加载当前页及相邻页。视频继续使用 Range 流式播放，不因收到消息自动保存完整文件。
...
缓存文件使用数据保护并排除系统备份；SQLite 账号缓存与 outbox 目录同样标记为可重建本地数据。撤回会清理原图、图片缩略图、视频封面、语音和文件预览；清除本地历史会同时清理已下载媒体，退出账号会清空全局媒体缓存以避免账号切换残留，但不会删除可靠 outbox。服务端可靠清理队列也会删除原文件及其缩略图旁路文件。
