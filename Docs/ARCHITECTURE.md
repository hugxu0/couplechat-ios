...
`ChatPersistence` 每次激活生成包含用户名的不可复用 scope。所有生产读写和关闭操作都校验该 scope；`AuthStore` 在登录生效和退出动作开始时立即推进 generation。旧账号尚未返回的 bootstrap、Sync、Socket、上传、ACK 或数据库任务因此只能被丢弃，不能触碰新账号的连接、时间线、共享状态或文件。
...
PostgreSQL 访问集中在 `server/src/db`。当前源码 schema 为 v36；v1–v36 migration 不得改写，后续只能追加。业务写入通过显式事务完成；`db/index.ts` 只做稳定 re-export。

情侣卡牌是独立的 cardGame 域：每日抽卡次数、抽卡幂等记录、个人卡库和情侣效果分别持久化在 v33 新增的四张表中，v34 让独立 migrator 创建的卡牌表恢复为与 `accounts` 一致的 Web 表属主。v35 将消息幂等唯一键限定到会话、账号、设备和 `clientId`，并仅为无设备旧会话保留独立兼容索引。v36 新增 `ai_reply_jobs`，把主人消息与待判断的 AI 回复任务放进同一事务。服务端以当前账号的 active identity 做权限边界；AI 任务还同时校验 requester、conversation 和 stored channel。
...
同一消息可能从 bootstrap、分页、Sync V2、Socket 或发送 ack 到达。服务端 `id` 用于去重，`clientId` 用于替换乐观消息；展示顺序固定使用 `(ts,id)`，已确认消息始终覆盖同 `clientId` 的 pending，不能依赖到达顺序。
...
- outbox 区分可自动恢复与必须手动重试的终止失败；重连只重放前者，用户点重试时才重置终止状态。排队或断网显示“等待发送”，只有当前真正执行上传/发送的项目显示“正在发送”。
- `message:send` ACK 返回的完整消息必须先写入 SQLite，再替换 UI pending 并推进最新窗口锚点；三者属于同一次有序提交，不能先发布成功消息后补锚点。
- ACK 只能更新仍存在且属于当前 scope 的 outbox 行，不能用 upsert 复活已经完成或属于旧账号的待发项。
- 聊天时间线可以在视觉上铺到输入区后方，供清透 Liquid Glass 直接采样；最后一条消息的安全边界必须由输入区完成布局后的真实 `frame` 换算成 `bottomInset`，不能估算键盘高度、输入框行数或面板组合高度。输入栏不接入会按滚动边缘状态额外改变模糊与透明度的容器交互；普通按钮和输入胶囊统一使用 clear glass，发送与录音按钮才切换为系统 prominent clear glass。pending 被 ACK 服务端消息替换、输入框高度变化或键盘动画时，是否跟随最新只由既有用户阅读意图决定；只有真实拖动/减速滚动、状态栏回顶或进入历史窗口能把它改为离底，程序化布局与 `reloadData` 产生的 `scrollViewDidScroll` 不能修改该意图。
- 服务端不信任客户端媒体 URL，只接受归属正确且未被占用的 `uploadId`。
- ack 丢失允许重试；同一设备、会话和 `clientId` 必须返回同一条完整消息，不能创建重复记录，也不能重复广播、调用 AI 或发送推送。不同设备可以独立使用相同 `clientId`。
- SQLite/bootstrap 已存在同一 `clientId` 的正式消息时，正式消息始终优先于 pending，并清理遗留 outbox；不能因 App 在 ACK 与本地清理之间退出而永久恢复成“正在发送”。
- 客户端监听网络路径变化；Wi-Fi、蜂窝或可达性切换时废弃旧 socket，重连成功后自动重放 outbox。
- 媒体上传完成后必须重新取得当前 socket，不能继续使用上传开始前捕获的旧连接。上传请求有空闲超时和资源级硬上限，避免单个挂起上传永久阻塞串行 outbox；发送方图片回填本地缓存异步执行，不能挡住 `message:send`。
...
消息搜索和历史窗口都使用 `(ts,id)` 复合游标。服务端按两列稳定排序，客户端持久化完整锚点、显式加载并按消息 id 去重；bootstrap 若检测到本地最新锚点与云端快照之间存在缺口，会用复合游标补齐。搜索命中在进入上下文前写入本地，但搜索列表分页与聊天上下文分页是两条独立链路。

## 媒体渐进读取

签名媒体和兼容媒体入口都提供单段 HTTP Range。除普通起止范围外，文件尾部的 suffix Range 是远程 MP4 快速读取 `moov` 元数据的必要契约；格式错误、多段或不可满足的 Range 必须返回带 `Content-Range: bytes */<size>` 的 `416`，不得静默退化为整段 `200`。客户端全屏播放只使用 `AVPlayer` 的分段缓冲和原生加载/失败状态，视频缩略图不得在后台回退为完整文件下载并与播放器争抢带宽。

聊天发送视频前使用系统导出器生成网络优化的 1080p H.264/AAC MP4，避免相机原始 4K HDR MOV 的超高码率和尾部索引阻塞首帧。历史原始视频仍优先使用 Range 流式播放；持续等待或 AVFoundation 失败时，播放器改为单次完整下载到临时文件后本地播放，退出页面即清理。播放器诊断只记录错误域与错误码，不记录签名媒体 URL。

新版 iOS 在后台用 ImageIO 从静态图片生成最长边 720 px 的 JPEG 预览，并把最多 512 KiB 的规范 Base64 字段置于原文件 part 之前；因此 iPhone 常见 HEIC 不依赖服务器图像库解码，原图仍按流式文件管线完整上传。服务端严格校验客户端预览；旧客户端没有预览时，以最多两个并发图像任务尝试生成同规格旁路文件，避免多图同时上传耗尽小型源站内存。

聊天气泡、相册网格和通用缓存图片先读已缓存原图，否则请求缩略图；缩略图不存在时回退原图，全屏浏览、下载和收藏仍使用原资源。缩略图与原资源共用同一个媒体 id 和短期签名权限，但使用独立路径与缓存条目。

客户端所有媒体缓存以媒体 host + path 为稳定身份，只对 `/media/up_*` 忽略每次刷新都会变化的 `sig` / `exp`；图片、语音和文件各自合并同一身份的并发加载，其他路径和查询参数仍参与隔离。这样 bootstrap、Socket 重连或回前台重新签发 URL 后不会重复下载同一媒体。

图片、视频封面、语音和文件预览都属于可重建缓存，不是离线事实源。图片磁盘上限 1 GiB、语音 256 MiB、文件预览 512 MiB；超过上限按最后访问时间回收到 85%。图片网络读取最多四并发，语音/文件最多两并发；全屏图片只加载当前页及相邻页。视频继续使用 Range 流式播放，不因收到消息自动保存完整文件。共享头像和贴纸在读取边界按 upload id 刷新短期签名，客户端用忽略 `sig/exp` 的稳定媒体身份合并；文件 GC 只把活跃头像、贴纸库和贴纸消息视为引用，墓碑不永久占用文件。

新版语音在 `message:send.meta.media.durationMs` 中携带整数毫秒时长，列表直接布局而不下载音频；旧语音没有该字段时显示通用“语音”标签，用户播放时才下载。自己发送成功的语音和文件从 outbox 原子提升到对应缓存，再删除待发副本，避免立即回源。

缓存文件使用数据保护并排除系统备份；SQLite 账号缓存与 outbox 目录同样标记为可重建本地数据。撤回会清理原图、图片缩略图、视频封面、语音和文件预览；清除本地历史会同时清理已下载媒体，退出账号会清空全局媒体缓存以避免账号切换残留，但不会删除可靠 outbox。服务端过期附件清理先在事务内锁定并删除未绑定元数据，同时写入可靠清理队列；消息/相册绑定中的行会被跳过，提交后才由队列删除原文件及缩略图旁路文件。签名媒体的 `sig/exp` 等同短期 bearer token，Fastify 请求日志必须去掉 query，边缘代理不得记录 `/media/` 的完整请求。
...
`/api/messages` 支持 `before`、`after`、`after + before`、`since` 和 `around`，并可附带 `beforeId` / `afterId` / `sinceId` 组成 `(ts,id)` 复合游标。旧客户端只传毫秒 `ts` 时仍可用；新客户端应同时传锚点 `id`。只有无方向游标的初始页计算 `total`，后续页返回 `null`，避免每次翻页扫描整段会话。
