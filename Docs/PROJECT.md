...
> 更新时间：2026-07-19。本文只记录当前产品、验证和生产事实，不记录开发过程。
...
| 服务端本地验证 | 2026-07-19 | `npm run check` 通过；覆盖大橘完整公聊窗口、单次模型写作入口、最小格式兜底，以及 schema v32 全部持久化表的备份策略覆盖 |
| 生产环境 | 2026-07-19 | 固定 RELEASE `1fb9ee14ddac7081a3a679338ce132fde04781bd` 已发布，前一 release `f9b2688e04795ea8d18dadcf0f252ba09ca66fbe` 保留回滚镜像，schema v32 未变化；发布入口的美国本机、私有 origin、公开入口三层健康、固定账号与 Socket 均通过；未执行 migration、备份、恢复或停写 |
...
- 大橘日记按作息日从共同聊天生成：服务端直接把上一作息日的完整 couple 公聊交给一次日记模型调用，由大橘自己挑选值得记住的片段、写观察和小心思；不再经过日总览、选材标签或局部摘录。大橘主页展示最新一篇，日记书架浏览历史，详情页可确认后按原聊天重新整理当日内容。正文保留自然段落，模型不可用时使用极小的事实受限兜底。
