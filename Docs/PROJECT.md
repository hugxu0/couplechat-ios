...
- main `f847a50`：四批修复与精简已合并（丢消息、已读归零、记忆召回、撤回假报错、
  抽取死锁、提醒备忘并发保护、持久层瘦身），CI（Xcode 26.3 编译 + SwiftLint +
  服务端检查 + 仓库检查）全绿，unsigned IPA 已构建（run `30201467493`）。
- **真机弱网验证尚未执行**；这批改的恰恰是弱网/离线路径，装机实测前不算完成。
- 生产运行 schema **v34**（release `7ce4e08`），源码是 **v36**。上线必须走
  [SERVER.md](SERVER.md) 的迁移步骤，不能用普通发布切换。
- 日本入口与美国生产环境健康检查正常（2026-07-26 核验）。
