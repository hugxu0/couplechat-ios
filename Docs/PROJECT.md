...
- 大橘日记按作息日从共同聊天生成：服务端直接把上一作息日的完整 couple 公聊交给一次日记模型调用，由大橘围绕一条真正放不下的暗线写观察、联想和小心思，不按时间逐项复述；标题使用有具体意象和余韵的诗性短句，正文目标为 450～600 个中文字符、4～5 个自然段；不再经过日总览、选材标签或局部摘录。大橘主页展示最新一篇，日记书架浏览历史，详情页可确认后按原聊天重新整理当日内容。模型不可用时使用极小的事实受限兜底。
