...
> 状态：执行中（当前交接点：R1.2 已验收，R2.1 进行中）
>
...
| 2 | R2.1 顶部视觉 Fixture | 进行中 | R1.2 |
| 2 | R2.2 原生导航栏 Spike | 待执行 | R2.1 |
| 2 | R2.3 替换聊天顶部 | 待执行 | R2.2 |
| 3 | R3.1 媒体 Viewer 协调器 | 待执行 | R2.3 |
| 3 | R3.2 交互式下拉退出 | 待执行 | R3.1 |
| 3 | R3.3 统一其他媒体入口 | 待执行 | R3.2 |
| 4 | R4.1 纯时间线构建器 | 待执行 | R3.3 |
| 4 | R4.2 滚动策略状态机 | 待执行 | R4.1 |
| 4 | R4.3 消息动作提供器 | 待执行 | R4.2 |
| 4 | R4.4 ChatTimelineController | 待执行 | R4.3 |
| 5 | R5.1 Repository/Persistence 协议 | 待执行 | R4.4 |
| 5 | R5.2 SQLite 单一所有者 | 待执行 | R5.1 |
| 5 | R5.3 拆分 MessageStore | 待执行 | R5.2 |
| 5 | R5.4 缩小 ChatStore | 待执行 | R5.3 |
| 6 | R6.1 扩充设计系统 | 待执行 | R5.4 |
| 6 | R6.2 逐页迁移 | 待执行 | R6.1 |
| 7 | R7.1 拆分后端测试 | 待执行 | R6.2 |
| 7 | R7.2 拆分数据库模块 | 待执行 | R7.1 |
| 7 | R7.3 降低业务耦合 | 待执行 | R7.2 |
| 7 | R7.4 可观测性与性能 | 待执行 | R7.3 |
| 8 | R8.1 发布前矩阵 | 待执行 | R7.4 |
| 8 | R8.2 上线与回滚准备 | 待执行 | R8.1 |
...
- 删除聊天顶部专用的 `topSafeGlass`、`TopBackdropBlur`、标题胶囊背景、手工文字阴影和顶部亮度切色。
- `ChatSurfaceTone` 如果仍被 composer 使用，只保留 composer 职责并改名，不能继续控制顶部。
- 顶部标题、状态和头像进入 R2.2 选定的同一个系统导航容器。
- 时间线可滚到导航材质后方；实际 content inset 由导航栏/安全区计算，不再使用 `safeTop + 58` 这类标题高度魔数。
