...
- **大橘日记**：调度器定期确保「上一作息日」日记（`ai_daily_diaries`）；材料只来自 couple 日总览/归档和所选话题时间点附近的少量 couple 主人消息，不读大橘发言或任一账号 AI 私聊。生成分两步：低温度选材先从候选话题中确定一条主线、至多一条相关陪衬及各一个相关决定/未决，再按 `lastAt` 在当前作息日边界内取有界原文恢复顺序与语气；写作只接收筛后的材料，正文以大橘第一人称写 2～4 个自然段并拒绝清单式输出。模型不可用、输出无效或正文过短时，服务端也只围绕本地选中的主线生成事实受限兜底。已有日记默认幂等返回，客户端详情页可确认后以显式 `force` 重写；REST 见 `GET/POST /api/v2/ai/diaries*`。
