...
- **大橘日记**：调度器定期确保「上一作息日」日记（`ai_daily_diaries`）；材料只来自 couple 日总览/归档，不读任一账号 AI 私聊。生成输入保留话题状态与多个要点、决定、未决和情绪线；正文以大橘第一人称写 3～5 个自然段，禁止补造时间、地点、动作、原话、因果或未决结果。模型不可用、输出无效或正文过短时，服务端按同一份材料生成事实受限的分段兜底，不再把摘要字段直接拼给用户。已有日记默认幂等返回，显式 `force` 才重写；REST 见 `GET/POST /api/v2/ai/diaries*`。
