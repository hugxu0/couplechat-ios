...
每轮初始输入包含北京时间、说话人、频道权限、窗口外摘要、最近 14 条聊天和当前问题。工具结果由 Agent 在同一轮继续处理，不重复塞进初始输入。
...
- `ai_memory_cursor`：每个频道已整理到的 `(cursor_ts, cursor_id)`；
...
运行 `npm run dev:cloud-db` 后打开 `http://127.0.0.1:8080/ai-debug`。调试页支持：
