...
- **大橘日记**：调度器定期确保「上一作息日」日记（`ai_daily_diaries`）；生成时直接读取该作息日完整的 couple 公聊（含大橘在公聊中的发言），不读任一账号 AI 私聊，也不再把日总览、选材标签或局部摘录当成正文材料。单次写作任务由大橘通读全部聊天后自行选择值得记住的两三幕，写大橘自己的观察、感受和思考；只要求不把主人潜台词冒充事实、不变成心理分析或建议。模型不可用时使用极小的事实受限兜底。已有日记默认幂等返回，客户端详情页可确认后以显式 `force` 重写；REST 见 `GET/POST /api/v2/ai/diaries*`。
