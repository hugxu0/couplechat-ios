...
`Docs/` 只保留当前有效的 9 份权威文档，全部平铺在本目录。这里承载产品状态、架构、契约和开发/部署方法；根目录及子目录 `AGENTS.md` 承载操作、安全和验证规则，不作为独立的产品状态事实源。历史状态、阶段报告和已废弃方案由 Git 记录，不再建立日期文件、ADR 目录或第二份接口说明。
...
| [CARD_GAME.md](CARD_GAME.md) | 情侣抽卡的当前第一版规则、卡片大类、品质和效果 |
