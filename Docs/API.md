...
列表与详情中的日记对象为 `{ id, coupleId, dayKey, title, body, source, createdAt, updatedAt }`，`body` 可包含以 `\n\n` 分隔的自然段。只读 couple 材料；无信号日返回 `404 empty_or_unavailable`。`ensure` 默认幂等返回已有日记；`force: true` 对指定日或昨日重新选材、生成并更新同一作息日记录，客户端详情页的“重新整理”确认操作使用此能力。
...
| 方法 | 路径 | 用途 |
|---|---|---|
| GET | /api/v2/card-game | 读取当前账号的每日次数、个人卡库、对方卡库摘要、进行中效果、出牌记录和卡片目录 |
...
每日每个账号最多 6 次翻牌，按 Asia/Shanghai 自然日计算；每张独立约六分之一翻中。快照含 `todayDraws`（今日已翻结果按顺序）供翻牌阵还原。卡片和效果属于当前情侣空间，服务端用事务和账号幂等键保证重复请求不会重复扣卡。时间型效果从成功使用时开始倒计时，addTime 增加倒计时、postpone 延后效果，copy 复制对方卡库的一张卡，qiankun 把当前对自己生效的效果转给对方。卡牌页在前台每 8 秒轮询 GET，当前版本不发送通知、不使用 Socket 或 Sync 事件。
