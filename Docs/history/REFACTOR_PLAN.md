...
> 历史文档：这是 2026-07-12 的 R0–R8/V1 发布计划，保留用于验收追溯；现行产品和技术路线已由 `../product/PRODUCT_PLAN.md`、`../architecture/V2_ARCHITECTURE.md` 接替。下文的“仅两位固定用户”“宠物暂不重构”和“只允许继续三项”均是当时边界，不是当前命令。
>
> 历史状态：主体功能已上线；R5.3/R5.4 的 facade 收敛与 R8.1 真机矩阵仍可作为维护参考
>
> 适用范围：iOS 客户端、Node.js 服务端、GitHub Actions 交付链路
>
> 当时产品约束：仅两位固定用户；设备均为 iPhone 17 / iPad 最新系统；允许重新登录和清空本地 SQLite；服务端聊天历史必须保留；AI、记录、提醒等已完成功能必须继续可用；宠物页暂不投入重构。
...
| 问题 | 当前证据 | 影响 | 处理方向 |
|---|---|---|---|
| 聊天顶部不是单一系统 | `ChatSessionScreen` 同时包含壁纸亮度采样、条件顶层 blur、手工黑白文字、文字阴影、标题胶囊和自定义 Liquid Glass | 调一个参数会破坏另一个状态，浅色/暗色壁纸结果不稳定 | 首选真正的系统导航栏/Toolbar；标题、材质、按钮由同一个原生容器负责 |
...
6. 协议任务必须同时检查服务端契约、Swift 契约、`../architecture/API.md` 和契约测试。
...
- 在 `../current/STATUS.md` 增加“本轮重构基线”小节，记录：
  - 键盘弹起和收起当前是保护行为。
...
- `ChatMediaViewerCoordinator` 负责 UIKit present/dismiss 与转场，不让 `ChatSessionScreen` 用 `fullScreenCover` 打开聊天媒体。
...
```text
你只执行 `REFACTOR_PLAN.md` 中的任务 Rx.x，不执行其他编号。

开始前：
1. 阅读该任务、`../architecture/SYSTEM_ARCHITECTURE.md`、`../development/DEVELOPMENT.md`。
2. 阅读任务点名的所有现有文件和测试。
...
- 两个真实账号在两部真机完成发布矩阵。
- `../current/STATUS.md`、`../architecture/SYSTEM_ARCHITECTURE.md`、`../development/DEVELOPMENT.md` 与最终代码一致。
