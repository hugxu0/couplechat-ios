...
> 最后私有运维记录：2026-07-17。域名、角色和端口已核对；上线前仍应现场检查 Cloudflare、Nginx、证书、容器、`RELEASE` 和 schema。本文不保存 IP、密码或任何密钥值。
...
- 日本冷数据不是实时复制，不能当作热备。
- migration、数据修复或冷切换前在私有 runbook 核对最近成功的本地/离机备份、恢复演练和 RPO/RTO；普通代码发布使用现行基线，不把完整恢复演练设为每次发布门槛。公开仓库不记录供应商、容量或实时失败状态。
