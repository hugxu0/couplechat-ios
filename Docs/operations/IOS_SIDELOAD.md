...
`build-ipa.yml` 会直接对同一个精确 commit 归档 unsigned IPA；它不重复执行服务端检查或模拟器 XCTest。等待运行 `conclusion=success`，并确认 `headSha` 完整等于 `$Sha`。服务端和日常 iOS 快速编译由默认验证 workflow 单独完成。
...
快速验证 workflow、metadata、SHA-256 和强制 commit 下载已在仓库实现，仍需推送后由第一次 GitHub Actions 成功运行完成远程验收。Apple 免费账号登录、2FA、首次信任、开发者模式和本机重新签名不能安全地放进 CI，继续由用户在受信电脑上完成。
