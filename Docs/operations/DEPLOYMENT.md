...
`server/.env.production.example` 的通用端口是 8080；RackNerd nginx 固定代理到 3000，因此生产 `.env` 必须显式设置 `PORT=3000`。
...
当前代码要求 schema v31。v26 增加关系/理解对基础记忆的来源关联；v27 清空不再使用的原始消息证据，并把按消息记录的忘记规则合并为卡片 key 规则；v28 自动归档重复的当前近况、关系和理解卡片，并通过唯一索引保证每个滚动分区只有一张当前卡；v29 增加每日推荐与账号级已读/隐藏状态；v30 增加大橘自有记忆的视角和类型字段，并更新滚动卡唯一索引；v31 为推荐增加开放式分类标签。发布必须先备份，再停旧 Web、运行一次独立 migrator，确认迁移记录写入后才启动新镜像。
...
备份只有出现“备份已原子发布”才可继续。脚本默认保留 7 天 daily、35 天 weekly，并在周日复制一份周备份；它会记录 schema、核心表计数、uploads manifest 与全部归档哈希。

未配置 `BACKUP_QUIESCE_HOOK` 时，数据库和 uploads 的一致性元数据明确记为 `best_effort`。生产希望强制短暂停写时应在主机提供绝对路径、非符号链接且可执行的 hook，并设置 `BACKUP_REQUIRE_QUIESCE=1`；hook 不属于仓库，缺失时脚本会拒绝继续。发布前至少检查本次 `METADATA` 的 `consistency_mode`。
...
还应在真机验证登录、Socket 重连、文字收发、图片/视频/表情上传、Range 媒体播放、多相册动态与全屏预览、计划、今日推荐/双方互荐、大橘和 AI 私聊。Bark 设置页可直接发送测试通知；通知入口应分别落到公聊、大橘私聊和提醒页面，URL scheme 为 `couplechat://`。
...
cron、`/root/backup/backup.sh`、`/usr/local/sbin/verify-couplechat-backup` 和 B2 上传逻辑不在仓库内，发布时必须在主机用 `crontab -l` 与实际脚本复核，不能只凭本文判断它们仍在运行。仓库中的 `scripts/verify-backup.sh` 会校验固定文件、真实恢复到随机临时数据库、比对 schema/表计数并抽样核对媒体；它要求 `VERIFY_ALLOWED_PREFIX` 和具备 `CREATEDB` 的 `VERIFY_DATABASE_URL`，默认拒绝超级用户。
...
gh run list --workflow build-ipa.yml --branch main --status success --limit 1
.\.github\scripts\download-latest-ipa.ps1 -RunId <main-success-run-id>
Get-FileHash build-artifacts\CoupleChat-latest.ipa -Algorithm SHA256
```

固定输出是 `build-artifacts/CoupleChat-latest.ipa`，不提交到 Git。不传 `-RunId` 时下载脚本不会限定 `main`，生产安装必须显式使用已核对 `headSha` 的 main 成功运行。
