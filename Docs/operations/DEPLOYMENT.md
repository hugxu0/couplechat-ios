...
- 仓库已有版本感知的备份、真实临时库恢复验证和健康检查等组件；备份轮转的历史代码缺陷已修复，但尚未在目标 Linux/真实生产副本演练。**目前仍没有经过端到端验证的一键首装/升级入口，也没有离机副本自动确认**。下文把当前安全边界和待实现的一键接口分开描述。
...
当前源码已经修复 [OPS-005/OPS-006](../current/KNOWN_ISSUES.md) 所记录的表清单漂移和“先删旧、后建新”问题，但还没有目标 Linux 与真实生产副本的演练证据，也不会自动复制或确认离机副本。因此脚本可以作为人工流程的组成部分，**不能单独充当自动发布门禁**。
...
代码层面，备份格式 v3 和恢复标记格式 v2 共用 v1–v31 版本感知的全表策略；恢复脚本会执行随机临时库真实 restore、schema/全表计数、两条关键序列和媒体抽样比对，并在成功删库后写标记。旧格式 v2 只允许降级验证且不会获得新标记。仍待完成的运维证据是：目标 Linux 失败注入、真实生产副本恢复演练、加密离机复制确认，以及把这些步骤接入尚未实现的一键部署入口。
