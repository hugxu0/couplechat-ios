...
当前生产已执行到 v24，而本工作树同样要求 v24；仍不要用当前工作树运行 `npm run dev:cloud-db` 直连生产：Web 进程已经强制 `RUN_MIGRATIONS=false`，会因 schema 版本不匹配安全退出，不会自动迁移。
...
在 schema 已迁移到 v24 的隔离调试库运行服务后访问：
